...
func AppendToMigration(domain string) {
	fileName := "migrations/migrations.go"
	...
	newSchema := strings.ToUpper(string(domain[0])) + domain[1:] + "Schema(),"
	new := fmt.Sprintf("%v\n\t\t%v%v", fileContent[:indexToAppend], newSchema, fileContent[indexToAppend:])
...
func Migration(c *cli.Context) {
	...
	if err != nil {
		log.Panic("File is Not Exist: %v", err)
	}
	...
	if err != nil {
		log.Panic("Unable to write file: %v", err)
	}
